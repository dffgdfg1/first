...
class VoltageCurrentPlotter:
    def __init__(self, root):
        ...
        # 保存上一轮的一级文件夹列表（用于模式2）
        self.last_parent_folders = []  # 保存上次选择的文件夹路径列表
    ...
    def _setup_ui(self):
        ...
        # 一键上传按钮
        ttk.Button(upload_frame, text="📂 一键上传文件夹",
                  command=self._one_click_upload).grid(row=2, column=3, padx=5, pady=3, sticky=tk.W)
        # 自动上传文件开关
        ttk.Checkbutton(upload_frame, text="自动上传文件",
                       variable=self.auto_add_files).grid(row=2, column=4, padx=10, pady=3, sticky=tk.W)
        # 智能分离曲线开关
        ttk.Checkbutton(upload_frame, text="智能分离曲线",
                       variable=self.smart_separate_curves).grid(row=2, column=5, padx=10, pady=3, sticky=tk.W)
    ...
    def _one_click_upload(self):
        """一键上传：从多个文件夹中提取指定位置的文件"""
        # 检查是否已生成日期段
        if not self.generated_dates:
            messagebox.showwarning("警告", "请先生成日期段！\n\n步骤：\n1. 点击'生成日期段'按钮\n2. 选择开始和结束日期\n3. 再使用一键上传功能")
            return

        date_count = len(self.generated_dates)

        # 创建对话框让用户选择模式
        mode_dialog = tk.Toplevel(self.root)
        mode_dialog.title("选择上传模式")
        mode_dialog.geometry("400x250")
        mode_dialog.transient(self.root)
        mode_dialog.grab_set()

        ttk.Label(mode_dialog, text="请选择上传模式：", font=("Microsoft YaHei", 12, "bold")).pack(pady=15)

        selected_mode = tk.StringVar(value="folder")

        ttk.Radiobutton(mode_dialog, text=f"模式1：从单个文件夹上传 {date_count} 个文件",
                       variable=selected_mode, value="folder").pack(anchor=tk.W, padx=30, pady=5)
        ttk.Label(mode_dialog, text="    （文件夹中有多个数据文件，按文件名排序匹配）",
                 foreground="gray", font=("Arial", 9)).pack(anchor=tk.W, padx=30)

        ttk.Radiobutton(mode_dialog, text=f"模式2：从 {date_count} 个文件夹中各提取1个文件",
                       variable=selected_mode, value="multi_folder").pack(anchor=tk.W, padx=30, pady=5)
        ttk.Label(mode_dialog, text="    （每个文件夹代表一天，从中提取指定位置的文件）",
                 foreground="gray", font=("Arial", 9)).pack(anchor=tk.W, padx=30)

        def on_confirm():
            mode_dialog.destroy()
            if selected_mode.get() == "folder":
                self._upload_from_single_folder()
            else:
                self._upload_from_multiple_folders()

        ttk.Button(mode_dialog, text="确定", command=on_confirm).pack(pady=20)
    ...
    def _upload_from_single_folder(self):
        """模式1：从单个文件夹上传多个文件"""
        date_count = len(self.generated_dates)
        ...
        if not all_files:
            messagebox.showwarning("警告", f"文件夹中没有找到支持的文件！\n\n支持的格式：CSV, TXT, XLSX, XLS")
            return
        ...
        if file_count != date_count:
            ...
            result = messagebox.askyesno(
                ...
                f"文件数量：{file_count} 个\n\n"
                f"是否继续？\n"
                f"（将按顺序匹配，多余的文件或日期将被忽略）"
            )
            if not result:
                return

        # 匹配文件到日期
        self._add_files_to_list(all_files, self.generated_dates)

    def _upload_from_multiple_folders(self):
        """模式2：从多个文件夹中各提取一个文件"""
        date_count = len(self.generated_dates)

        # 创建主对话框
        main_dialog = tk.Toplevel(self.root)
        main_dialog.title("从多个文件夹提取文件")
        main_dialog.geometry("500x400")
        main_dialog.transient(self.root)
        main_dialog.grab_set()

        # 标题
        ttk.Label(main_dialog, text="从多个文件夹中提取文件",
                 font=("Microsoft YaHei", 12, "bold")).pack(pady=10)

        # 文件夹信息显示
        info_frame = ttk.LabelFrame(main_dialog, text="当前文件夹信息", padding="10")
        info_frame.pack(fill=tk.BOTH, padx=20, pady=10)

        folder_info_label = ttk.Label(info_frame, text="未选择文件夹", foreground="gray")
        folder_info_label.pack()

        # 按钮区域
        button_frame = ttk.Frame(main_dialog)
        button_frame.pack(pady=10)

        # 选择新文件夹按钮
        ttk.Button(button_frame, text="📂 选择多个文件夹",
                  command=lambda: self._select_parent_folder(folder_info_label, main_dialog)).pack(side=tk.LEFT, padx=5)

        # 恢复上一轮按钮
        restore_btn = ttk.Button(button_frame, text="🔄 恢复上一轮文件夹",
                                command=lambda: self._restore_last_folders(folder_info_label, main_dialog))
        restore_btn.pack(side=tk.LEFT, padx=5)

        # 清除当前选择按钮
        ttk.Button(button_frame, text="🗑️ 清除当前选择",
                  command=lambda: self._clear_current_folders(folder_info_label, main_dialog)).pack(side=tk.LEFT, padx=5)

        # 如果没有上一轮数据，禁用恢复按钮
        if not self.last_parent_folders:
            restore_btn.config(state='disabled')

        # 索引位置选择区域
        position_frame = ttk.LabelFrame(main_dialog, text="选择文件位置索引", padding="10")
        position_frame.pack(fill=tk.BOTH, padx=20, pady=10)

        ttk.Label(position_frame, text="从每个文件夹中提取第几个文件？",
        ...
        ttk.Label(input_frame, text="位置索引:").pack(side=tk.LEFT, padx=5)
        position_var = tk.StringVar(value="1")
        position_entry = ttk.Entry(input_frame, textvariable=position_var, width=10)
        position_entry.pack(side=tk.LEFT, padx=5)

        # 快速索引按钮
        quick_btn_frame = ttk.Frame(position_frame)
        quick_btn_frame.pack(pady=10)

        ttk.Label(quick_btn_frame, text="快速选择:").pack(side=tk.LEFT, padx=5)
        for i in range(1, 6):
            ttk.Button(quick_btn_frame, text=f"第{i}个", width=6,
                      command=lambda idx=i: position_var.set(str(idx))).pack(side=tk.LEFT, padx=2)

        # 提取按钮
        extract_btn = ttk.Button(main_dialog, text="✅ 开始提取并添加到列表",
                                command=lambda: self._extract_files_from_folders(main_dialog, position_var),
                                state='disabled')
        extract_btn.pack(pady=10)

        # 删除文件按钮
        delete_frame = ttk.Frame(main_dialog)
        delete_frame.pack(pady=5)

        ttk.Button(delete_frame, text="🗑️ 删除选中的文件",
                  command=self._remove_selected).pack(side=tk.LEFT, padx=5)
        ttk.Button(delete_frame, text="🗑️ 清空所有文件",
                  command=self._clear_all).pack(side=tk.LEFT, padx=5)

        # 保存提取按钮引用，以便后续启用
        main_dialog.extract_btn = extract_btn
        main_dialog.all_folders = []

        # 居中显示
        main_dialog.update_idletasks()
        x = (main_dialog.winfo_screenwidth() // 2) - (main_dialog.winfo_width() // 2)
        y = (main_dialog.winfo_screenheight() // 2) - (main_dialog.winfo_height() // 2)
        main_dialog.geometry(f"+{x}+{y}")

        self.root.wait_window(main_dialog)

    def _select_parent_folder(self, info_label, dialog):
        """选择多个文件夹（每个文件夹代表一天）"""
        date_count = len(self.generated_dates)

        # 使用文件对话框选择多个文件夹
        messagebox.showinfo("提示", f"请选择 {date_count} 个文件夹\n\n每个文件夹代表一天的数据")

        # 让用户选择多个文件夹
        selected_folders = []
        for i in range(date_count):
            folder = filedialog.askdirectory(title=f"选择第 {i+1}/{date_count} 个文件夹（日期: {self.generated_dates[i]}）")
            if not folder:
                if i == 0:
                    # 如果第一个就取消了，直接返回
                    return
                    ...
                    # 询问是否继续
                    result = messagebox.askyesno("提示", f"已选择 {i} 个文件夹，是否继续？")
                    if result:
                        continue
                    else:
                        break
        ...
        # 保存到上一轮
        self.last_parent_folders = selected_folders.copy()

        # 更新信息显示
        info_label.config(text=f"已选择 {folder_count} 个文件夹",
                         foreground="green")

        # 保存到对话框
        dialog.all_folders = selected_folders

        # 启用提取按钮
        dialog.extract_btn.config(state='normal')

        self._log(f"已选择 {folder_count} 个文件夹", "INFO")

        # 检查数量匹配
        if folder_count != date_count:
            messagebox.showwarning(
                "数量不匹配",
                f"文件夹数量与日期数量不匹配！\n\n"
                f"日期数量：{date_count} 天\n"
                f"文件夹数量：{folder_count} 个\n\n"
                f"将按顺序匹配，多余的文件夹或日期将被忽略"
            )

    def _restore_last_folders(self, info_label, dialog):
        """恢复上一轮的文件夹"""
        if not self.last_parent_folders:
            messagebox.showinfo("提示", "没有可恢复的文件夹！")
            return

        date_count = len(self.generated_dates)
        folder_count = len(self.last_parent_folders)

        # 恢复文件夹列表
        dialog.all_folders = self.last_parent_folders.copy()

        # 更新信息显示
        info_label.config(text=f"已恢复 {folder_count} 个文件夹",
                         foreground="blue")

        # 启用提取按钮
        dialog.extract_btn.config(state='normal')

        self._log(f"已恢复上一轮文件夹列表（{folder_count} 个文件夹）", "INFO")
    ...
    def _clear_current_folders(self, info_label, dialog):
        """清除当前选择的文件夹"""
        dialog.all_folders = []
        info_label.config(text="未选择文件夹", foreground="gray")
        dialog.extract_btn.config(state='disabled')
        self._log("已清除当前选择的文件夹", "INFO")

    def _extract_files_from_folders(self, dialog, position_var):
        """从文件夹中提取文件"""
        if not dialog.all_folders:
        ...
        try:
            ...
            if position < 1:
                messagebox.showerror("错误", "位置索引必须大于等于1！")
                return

            # 从每个文件夹中提取指定位置的文件
            supported_extensions = ['.csv', '.txt', '.xlsx', '.xls']
            date_count = len(self.generated_dates)
            folder_count = len(dialog.all_folders)
            match_count = min(folder_count, date_count)

            result_files = []
            skipped_folders = []

            for i in range(match_count):
                folder_path = dialog.all_folders[i]
                ...
                try:
                    ...
                    self._log(f"读取文件夹 {folder_name} 失败: {str(e)}", "ERROR")
                    skipped_folders.append(folder_name)
                    continue

                if not files_in_folder:
                    self._log(f"文件夹 {folder_name} 中没有找到支持的文件 (支持: CSV, TXT, XLSX, XLS)", "WARNING")
                    skipped_folders.append(folder_name)
                    continue
                ...
                # 输出文件列表用于调试
                self._log(f"文件夹 {folder_name} 中的文件（共{len(files_in_folder)}个）:", "INFO")
                for idx, f in enumerate(files_in_folder, 1):
                    self._log(f"  索引{idx}: {os.path.basename(f)}", "INFO")

                # 提取指定位置的文件
                if position <= len(files_in_folder):
                    result_files.append(files_in_folder[position - 1])
                    self._log(f"✓ 从 {folder_name} 提取索引{position}: {os.path.basename(files_in_folder[position - 1])}", "SUCCESS")
                else:
                    self._log(f"文件夹 {folder_name} 中只有 {len(files_in_folder)} 个文件，无法提取第 {position} 个", "WARNING")
                    skipped_folders.append(folder_name)

            if result_files:
                # 添加文件到列表
                self._add_files_to_list(result_files, self.generated_dates)
                # 不关闭对话框，方便用户继续提取其他索引位置的文件
                success_msg = f"成功提取并添加了 {len(result_files)} 个文件！"
                if skipped_folders:
                    success_msg += f"\n\n跳过了 {len(skipped_folders)} 个文件夹（无文件或文件不足）"
                success_msg += "\n\n可以继续选择其他索引位置提取更多文件。"
                messagebox.showinfo("成功", success_msg)
            else:
                error_msg = "没有提取到任何文件！"
                if skipped_folders:
                    error_msg += f"\n\n所有 {len(skipped_folders)} 个文件夹都被跳过了。\n请检查文件夹中是否有支持的文件格式。"
                messagebox.showwarning("警告", error_msg)

        except ValueError:
            messagebox.showerror("错误", "请输入有效的数字！")

    def _add_files_to_list(self, files, dates):
        """将文件添加到列表，匹配到日期"""
        match_count = min(len(files), len(dates))
        added_count = 0
        ...
        for i in range(match_count):
            ...
            if any(f[0] == file_path for f in self.file_list):
                self._log(f"文件已存在，跳过: {file_name}", "WARNING")
                continue
        ...
        # 显示结果
        if added_count > 0:
            self._log(f"一键上传成功：添加了 {added_count} 个文件", "SUCCESS")
            self._log(f"当前共加载 {len(self.file_list)} 个文件", "INFO")
            messagebox.showinfo("成功", f"成功添加 {added_count} 个文件到列表！")
        else:
            self._log("没有新文件被添加（可能都已存在）", "WARNING")
