...
class EChartsEditor:
    ...
    def create_widgets(self):
        ...
        ttk.Button(toolbar_row1, text="1. 批量上传HTML", command=self.load_multiple_files).pack(side=tk.LEFT, padx=5)
        ttk.Button(toolbar_row1, text="2. 导出Excel", command=self.export_excel).pack(side=tk.LEFT, padx=5)
        ttk.Button(toolbar_row1, text="3. 导入Excel", command=self.import_excel).pack(side=tk.LEFT, padx=5)
        ttk.Button(toolbar_row1, text="4. 生成新HTML", command=self.generate_html).pack(side=tk.LEFT, padx=5)

        # 第二行：附加功能按钮
        toolbar_row2 = ttk.Frame(toolbar_container)
        toolbar_row2.pack(side=tk.TOP, fill=tk.X, pady=(0, 3))

        ttk.Button(toolbar_row2, text="全局操作", command=self.global_operations).pack(side=tk.LEFT, padx=5)
        ...
        toolbar_row3.pack(side=tk.TOP, fill=tk.X)

        ttk.Label(toolbar_row3, text="搜索数值:").pack(side=tk.LEFT, padx=(5, 2))
        self.search_entry = ttk.Entry(toolbar_row3, width=15)
    ...
    def search_value(self):
        ...
        if not search_text:
            messagebox.showwarning("警告", "请输入要搜索的数值！")
            return

        try:
            search_value = float(search_text)
        except ValueError:
        ...
        for item in tree.get_children():
            ...
            try:
                item_value = float(values[3])  # 第4列是数值列
                # 模糊匹配：在正负3范围内都算匹配
                if abs(item_value - search_value) <= tolerance:
    ...
    def fill_missing_data(self):
        ...
        # 获取当前选中的标签页索引
        current_tab = self.notebook.index(self.notebook.select())
        if current_tab >= len(self.series_data):
            return

        series = self.series_data[current_tab]
        data = series['data']

        if len(data) < 2:
            messagebox.showinfo("提示", "数据点太少，无法检测缺失数据")
            return
        ...
        for i in range(len(time_diffs)):
            if time_diffs[i] > median_diff * 2:
                gaps.append({
                    ...
                    'missing_count': int((time_diffs[i] / median_diff) - 1)
                })
        ...
        # 居中显示
        fill_dialog.update_idletasks()
        x = (fill_dialog.winfo_screenwidth() // 2) - (fill_dialog.winfo_width() // 2)
    ...
    def global_operations(self, restore_params=None):
        ...
        ttk.Radiobutton(peak_clipping_params_frame, text="自定义值", variable=peak_clipping_method_var, value="custom").grid(row=5, column=0, sticky=tk.W, padx=2)

        # 自定义值输入框和智能填充按钮
        peak_custom_value_entry = ttk.Entry(peak_clipping_params_frame, width=10)
        peak_custom_value_entry.grid(row=5, column=1, sticky=tk.W, padx=2)

        def smart_fill_average():
            """智能计算平均值并填充到自定义值输入框"""
            try:
                data_type = peak_clipping_type_var.get()
                start_str = peak_clipping_start_entry.get().strip()
                end_str = peak_clipping_end_entry.get().strip()
                max_str = peak_clipping_max_entry.get().strip()
                min_str = peak_clipping_min_entry.get().strip()

                if not start_str or not end_str:
                    messagebox.showwarning("警告", "请先设置作用范围")
                    return

                if not max_str or not min_str:
                    messagebox.showwarning("警告", "请先设置最大/最小阈值")
                    return
                ...
                series = self.html_files[self.current_file_index]['series']

                # 根据数据类型获取对应的数据
                if data_type == "voltage":
                    voltage_series = [s for s in series if '电压' in s.get('name', '')]
                    if not voltage_series:
                        messagebox.showwarning("警告", "未找到电压数据")
                        return
                    data = voltage_series[0]['data'][start_idx:end_idx+1]
                else:
                    current_series = [s for s in series if '电流' in s.get('name', '')]
                    if not current_series:
                        messagebox.showwarning("警告", "未找到电流数据")
                        return
                    data = current_series[0]['data'][start_idx:end_idx+1]

                if len(data) == 0:
                    messagebox.showwarning("警告", "没有可用的数据")
                    return

                # 提取数值（处理[timestamp, value]格式）
                values = []
                for item in data:
                ...
                if len(values) == 0:
                    messagebox.showwarning("警告", "没有有效的数据点")
                    return

                # 筛选正常数据点（只排除超过阈值的异常峰值，保留包括0在内的正常值）
                normal_values = [v for v in values if min_threshold <= v <= max_threshold]

                if len(normal_values) == 0:
                    messagebox.showwarning("警告", "所有数据点都超出阈值范围，无法计算平均值")
                    return

                # 计算正常数据的平均值
                avg_value = sum(normal_values) / len(normal_values)
                peak_custom_value_entry.delete(0, tk.END)
                peak_custom_value_entry.insert(0, f"{avg_value:.2f}")

            except Exception as e:
                messagebox.showerror("错误", f"智能填充失败: {str(e)}")

        ttk.Button(peak_clipping_params_frame, text="智能填充平均值", command=smart_fill_average).grid(row=5, column=2, columnspan=2, sticky=tk.W, padx=2)

        ttk.Label(peak_clipping_params_frame, text="（异常值处理方式）", foreground='gray').grid(row=6, column=0, columnspan=4, sticky=tk.W, pady=2)
        ...
        ttk.Label(cycle_copy_params_frame, text="自定义采样率:").grid(row=7, column=0, sticky=tk.W, pady=2)
        cycle_copy_custom_rate_entry = ttk.Entry(cycle_copy_params_frame, width=15)
        cycle_copy_custom_rate_entry.grid(row=7, column=1, padx=2)
        ...
        if restore_params:
            ...
            cycle_copy_custom_rate_entry.delete(0, tk.END)
            cycle_copy_custom_rate_entry.insert(0, restore_params.get('cycle_copy_custom_rate', '0'))
        ...
        def apply_operation():
            try:
              ...
              # 计算源段和目标段的长度
                    ...
                    # 如果是自定义值方式，获取自定义值
                    custom_value = None
                    if peak_clipping_method == "custom":
                        custom_value_str = peak_custom_value_entry.get().strip()
                        if not custom_value_str:
                            messagebox.showerror("错误", "请输入自定义值或使用智能填充按钮！", parent=op_window)
                            return
                    ...
                    custom_sample_rate = float(cycle_copy_custom_rate_entry.get())  # 自定义采样率（点/秒）
                    ...
                    for file_info in files_to_process:
                        ...
                        for series_idx in series_indices:
                            ...
                            # 提取周期数据的数值（根据是否使用自定义采样率决定处理方式）
                            cycle_values = []

                            if custom_sample_rate > 0:
                                # 使用自定义采样率：先对源数据降采样（按时间分组求平均值）
                                print(f"  使用自定义采样率: {custom_sample_rate} 点/秒")

                                # 计算源数据的采样率（点/秒）
                                source_sample_rate = 1000.0 / avg_time_interval  # avg_time_interval是毫秒
                                print(f"  源数据采样率: {source_sample_rate:.2f} 点/秒")

                                # 计算降采样比例
                                if custom_sample_rate >= source_sample_rate:
                                    # 自定义采样率大于等于源采样率，不需要降采样
                                    print(f"  自定义采样率 >= 源采样率，不进行降采样")
                                    for i in range(actual_source_start, actual_source_end + 1):
                                        if isinstance(data[i], list) and len(data[i]) >= 2:
                                            value = data[i][1]
                                            cycle_values.append(value if value is not None else 0)
                                        else:
                                            cycle_values.append(0)
                                else:
                                    # 需要降采样：按时间分组求平均值
                                    group_interval_ms = 1000.0 / custom_sample_rate  # 每组的时间间隔（毫秒）
                                    print(f"  降采样分组间隔: {group_interval_ms:.2f} ms")

                                    # 按时间分组
                                    start_timestamp = data[actual_source_start][0]
                                    current_group_start = start_timestamp
                                    current_group_values = []

                                    for i in range(actual_source_start, actual_source_end + 1):
                                        if isinstance(data[i], list) and len(data[i]) >= 2:
                                            timestamp = data[i][0]
                                            value = data[i][1] if data[i][1] is not None else 0

                                            # 判断是否属于当前组
                                            if timestamp < current_group_start + group_interval_ms:
                                                current_group_values.append(value)
                                            else:
                                                # 当前组结束，计算平均值
                                                if current_group_values:
                                                    avg_value = sum(current_group_values) / len(current_group_values)
                                                    cycle_values.append(avg_value)

                                                # 开始新组
                                                current_group_start += group_interval_ms
                                    ...
                                    print(f"  降采样后周期数据点数: {len(cycle_values)} (原始: {actual_cycle_length})")

                                # 更新时间间隔为自定义采样率对应的间隔
                                avg_time_interval = 1000.0 / custom_sample_rate
                            ...
                            cycle_index = 0
                            cycle_values_length = len(cycle_values)  # 使用实际的cycle_values长度

                            print(f"开始生成循环数据 - 起始时间戳: {current_timestamp}, 结束时间戳: {end_timestamp_ms}, 时间间隔: {avg_time_interval}, 周期数据点数: {cycle_values_length}")

                            while current_timestamp < end_timestamp_ms:
                                # 循环使用周期数据的数值
                                value = cycle_values[cycle_index % cycle_values_length]
                ...
                saved_params = {
                    ...
                    'cycle_copy_custom_rate': cycle_copy_custom_rate_entry.get() if cycle_copy_custom_rate_entry.get() else '0',
                    # 数据插入参数
    ...
    def batch_generate(self):
        ...
        ttk.Checkbutton(params_frame, text="批削峰（替换超过峰值的数据为平均值）:",
                       variable=peak_clip_enabled_var).grid(row=11, column=0, sticky=tk.W, pady=5, columnspan=2)

        ttk.Label(params_frame, text="峰值阈值:").grid(row=11, column=2, sticky=tk.W, pady=5)
        ...
        ttk.Label(params_frame, text="0值处理:").grid(row=12, column=0, sticky=tk.W, pady=5)
        ttk.Radiobutton(params_frame, text="默认（跳过0值）", variable=zero_handling_var,
                       value="default").grid(row=12, column=1, padx=5, sticky=tk.W)
        ttk.Radiobutton(params_frame, text="替换为null", variable=zero_handling_var,
                       value="to_null").grid(row=12, column=2, padx=5, sticky=tk.W)
        ttk.Radiobutton(params_frame, text="作为实际数据", variable=zero_handling_var,
                       value="as_data").grid(row=12, column=3, padx=5, sticky=tk.W)

        # 开关机次数统计选项
        power_cycle_count_var = tk.BooleanVar(value=saved_params.get("power_cycle_count", False))
        ttk.Checkbutton(params_frame, text="生成开关机次数统计（显示在HTML右上角，检测电流变化）",
                       variable=power_cycle_count_var).grid(row=13, column=0, columnspan=5, sticky=tk.W, pady=5)

        # 平滑输出选项
        smooth_enabled_var = tk.BooleanVar(value=saved_params.get("smooth_enabled", False))
        ttk.Checkbutton(params_frame, text="平滑输出（移动平均）:",
                       variable=smooth_enabled_var).grid(row=14, column=0, sticky=tk.W, pady=5)
        ttk.Label(params_frame, text="窗口大小:").grid(row=14, column=1, sticky=tk.W, pady=5)
        smooth_window_entry = ttk.Entry(params_frame, width=15)
        smooth_window_entry.grid(row=14, column=2, padx=5, pady=5)
